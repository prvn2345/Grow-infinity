    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-16">
      ...
      <nav className="bg-white border-b border-slate-200 px-4 py-3 md:px-6 md:py-4 flex justify-between items-center shadow-sm w-full">
        <div className="flex items-center space-x-1.5 md:space-x-2 min-w-0">
          ...
          <span className="hidden sm:inline-block bg-slate-100 text-slate-600 px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-wide uppercase border border-slate-200 flex-shrink-0">Parent Browser</span>
        </div>
        <div className="flex items-center space-x-2 md:space-x-6 flex-shrink-0">
          <button 
            onClick={() => router.push("/dashboard/parent")}
            className="text-slate-600 hover:text-blue-600 text-xs md:text-sm font-semibold transition"
          >
            <span className="hidden sm:inline">My Dashboard</span><span className="sm:hidden">Dashboard</span>
          </button>
          <button 
